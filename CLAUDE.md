...
| `council.py` | `council_plan` / `council_review`: seat the roster (validated bases; reviewers ≠ producer base), **Foundation pre-flight grounding** → blind round (grounded by the manifest) → reveal/resolution, evidence-gated `publish_final`. `tier=` scales the roster. |
