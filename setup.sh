...
DATABASE="${DCM_NEO4J_DATABASE:-neo4j}"
ok=1
...
python3 - <<PY 2>/dev/null && echo "[ok] mesh reachable at $URI database=$DATABASE" || { echo "[BLOCKED] mesh not reachable at DCM_NEO4J_URI=$URI DCM_NEO4J_DATABASE=$DATABASE -> start Neo4j or set DCM_NEO4J_URI/DCM_NEO4J_DATABASE (+ DCM_NEO4J_USER/PASSWORD if non-loopback)"; ok=0; }
