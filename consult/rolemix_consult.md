...
## Ground truth — the experiment + what's built

### The experiment (decided architecture)
...
The architecture is fixed (above); what is undetermined is the **mix and mechanics within it**: which expert functions earn a seat and how they split/fuse into 6–12, how they staff across the 4 CLIs for genuine decorrelation, how they deliberate in real time without manufacturing conformity, where the boundary sits between the deterministic gate floor and the semantic council, and how ISMA/Git grounding is wired so prior foundations are actually used. We have a draft (`design/EXPERIMENT.md`) but have not validated that draft against the failure modes — the mix is genuinely open.
