...
| `cli_adapter.py` | runs a real CLI as a mesh expert (`cli_expert`); prompts via stdin / `--prompt-file` (never argv). Untrusted sessions refuse a bare acting CLI — require a `sandbox` runner. |
